...
// KOREKSI (subtab pengajuan koreksi qty dari Gudang) DIHAPUS dari sini (13 Sep 2026, "approval
// koreksi qty diarahkan ke Admin") - dulu diputuskan siapa pun yang login di divisi peminta
// (target_divisi), sekarang SELALU ke Admin (PermintaanAdminTab.tsx di vista-teknik, tab
// "Koreksi Qty"). Alur PENGAJUAN-nya (Gudang klik "Ajukan Koreksi Qty" di RiwayatGudangTab.tsx)
// TIDAK berubah sama sekali - cuma titik PERSETUJUAN yang pindah, jadi gak ada yang perlu
// disentuh di form/riwayat operator di sini selain jenis "KOREKSI" ini sendiri (gak pernah lagi
// ada yang perlu diputuskan operator/divisi peminta).
...
export function PermintaanView({user,registerBackHandler,navTarget,onNavTargetConsumed}:{user:any,registerBackHandler?:(fn:(()=>boolean)|null)=>void,navTarget?:{id:number,jenis:string,tanggal:string}[]|null,onNavTargetConsumed?:()=>void}){
  ...
  useEffect(()=>{
    // HUTANG bukan kategori komponen_master asli (cuma BBMB/BBMU) - skip fetch, gak ada
  ...
  const fetchRiwayat=async()=>{
    if(jenisTab==="HUTANG")return; // Hutang punya fetch+state sendiri
  ...
  // State/fetch/putuskan KOREKSI (dulu di sini, per-divisi) DIHAPUS (13 Sep 2026, "approval
  // koreksi qty diarahkan ke Admin") - keputusan approve/tolak sekarang murni di
  // PermintaanAdminTab.tsx (vista-teknik), gak ada lagi yang perlu diputuskan di app operator.
  ...
  return(
    <div style={{padding:16}} className="fi">
      ...
      {/* HUTANG (5 Sep 2026) - subtab terpisah, read-only+approval, gak ada form pengajuan sama
          sekali (muncul otomatis dari Gudang, operator gak "minta" secara manual di sini). */}
...
// KoreksiBlock (UI approval per-divisi) DIHAPUS (13 Sep 2026) - lihat komentar di atas type Jenis.
