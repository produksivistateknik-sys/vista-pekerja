...
// ─────────────────────────────────────────────────────────────────────────────
// PDF VIEWER (1 Sep 2026) - render custom per-halaman ke <canvas> via pdfjs-dist buat SEMUA
// platform (Android maupun iOS). Sempat dicoba <iframe> native, dua kali: pertama untuk semua
// platform, lalu percobaan kedua khusus iOS aja (dengan asumsi Safari iOS beda dari Chrome
// Android) - TAPI dikonfirmasi user coba langsung di iPhone: SAMA SAJA maksa download, gak
// nampilin inline. Jadi iframe TERBUKTI gak reliable di kedua platform mobile utama, bukan cuma
// Android - <canvas> custom (gak bergantung dukungan browser buat PDF-di-iframe sama sekali)
// satu-satunya jalan yang dijamin tampil in-app tanpa download di HP manapun.
//
// Vista Teknik (dipakai di laptop/desktop) TETAP pakai <iframe> native - jalan normal di sana,
// beda kasus dari mobile.
//
...
// Sudah dicek: proxy ini betul dukung Range request (206 Partial Content), bukan sumber lambat.
//
// Render 2 tahap (resolusi rendah dulu biar langsung muncul, baru upgrade kualitas penuh di
// background) + pinch-zoom manual (2 jari, sama teknik kayak FotoZoomViewerPekerja.tsx - browser
// gak bisa native zoom karena user-scalable=no di index.html) + tombol zoom +/- fallback +
// safety-net timeout (kalau pdfjs-dist macet, dilaporkan kadang kejadian di iPhone, tetap ada
// jalan keluar pesan error + buka tab baru, bukan loading selamanya).
// ─────────────────────────────────────────────────────────────────────────────
...
export function PdfViewerPekerja({url,title,subtitle,onBack}:{url:string,title:string,subtitle?:string,onBack:()=>void}){
  ...
  useEffect(()=>{
    ...
    // Safety-net (1 Sep 2026, dilaporkan di iPhone/Safari): pdfjs-dist kadang MACET TOTAL (gak
    // resolve, gak reject) alih-alih gagal dengan error - user cuma liat "Memuat PDF..." tanpa
    // akhir. Timeout ini mastiin tetap ada jalan keluar (pesan error + link buka tab baru).
  ...
  return(
    <div style={{padding:"12px 12px 20px"}}>
      ...
      {/* BUG FIX (1 Sep 2026, dilaporkan di tablet: "geser cuma bisa ke kiri, ke kanan gak
          sampe full") - kuirk flexbox yang cukup dikenal: justify-content:center pada container
          overflow:auto bikin salah satu sisi overflow-nya gak kejangkau scroll (browser ngitung
          area scroll dari posisi "center", bukan dari batas asli konten). Ganti dari
          display:flex+justifyContent:center ke <canvas> di-center pakai margin:0 auto biasa -
          teknik lama tapi gak punya kuirk itu, overflow di kedua sisi kejangkau scroll normal. */}
