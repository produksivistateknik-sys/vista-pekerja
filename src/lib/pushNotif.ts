...
// Minta izin + subscribe + simpan endpoint/keys ke push_subscriptions - keyed by `divisi` (KEY
// DIVISI_CONFIG, mis. "mekanik"/"painting", BUKAN admin_username - device Vista Pekerja login
// pakai password BERSAMA per sub-bagian, jadi identitasnya divisi/device, bukan individu).
// endpoint UNIQUE - device yang sama subscribe ulang otomatis upsert, gak numpuk baris duplikat.
export const subscribeToPush = async (divisi: string): Promise<{ success: boolean; error?: string }> => {
